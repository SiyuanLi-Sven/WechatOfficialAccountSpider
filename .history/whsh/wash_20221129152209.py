...
def main(account_name='读书杂志'):
    ...
    for index, row in df1.iterrows():
        if row['year_month'] in df0['year_month'].values:  # 如果这个月有发文
            ...
            df1.loc[index, 'monthly_likes_sum_first_post'] = df_temp['like_num'].sum()
            ...
            df_temp = df0[df0['year_month'] == row['year_month']]
            df_temp = df_temp[df_temp['post_index'] == 1]
            df1.loc[index, 'monthly_read_first_post'] = df_temp['like_num'].sum()
            ...
            df1.loc[index, 'monthly_titles'] = '☀'.join(lis_temp)
    print(df1)
...
if __name__ == '__main__':
    ...
    for i in lis_address:
        print('正在开始：{}'.format(i))
        if i in ['temp' ,'云南社会科学.csv','人民论坛.csv','内蒙古社会科学.csv','天津社会科学.csv','新疆社会科学.csv','江汉论坛.csv','江淮论坛杂志社.csv']:
            continue
        i = i.replace('.csv', '')
        try:
            main(i)
